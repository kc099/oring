...
subtraction, detects contours, computes geometric metrics, and
classifies as PASS / REWORK / REJECT.  On PASS, runs a DL defect
detector (Mask R-CNN or YOLO) for confirmation.

Metrics kept (8 total):
...
def compute_resolution_scale(img_w: int, img_h: int) -> float:
    """Scale factor relative to REFERENCE_RESOLUTION."""
...
METRIC_DEFS = [
    ...
    # ── REJECT metrics (concentricity — unfixable) ───────────────────────
...
def load_good_stats(csv_path: Path) -> Optional[Dict]:
    ...
    # Only load columns relevant to METRIC_DEFS
...
def load_tuned_thresholds(json_path: Path) -> Optional[Dict[str, Dict]]:
    """Load tuned thresholds from JSON — only for metrics in METRIC_DEFS."""
...
# ═══════════════════════════════════════════════════════════════════════════
#  Image processing (streamlined — only computes the 8 METRIC_DEFS)
# ═══════════════════════════════════════════════════════════════════════════
...
def measure_oring(image: np.ndarray,
    ...
    """Compute the 8 geometric metrics + data needed for overlay drawing.

    Removed: wall-thickness ray tracing, edge_clearance, annular_area,
    and other unused derived metrics — significantly faster.
    """
    ...
    return {
        ...
        "outer_radial_std":  radial_std(outer),
        "inner_radial_std":  radial_std(inner),
        # ── For overlay drawing ──────────────────────────────────────────
        "mask":              mask,
        "outer_contour":     outer,
        "inner_contour":     inner,
...
class InspectionGUI(QMainWindow):
    """Main inspection window — streamlined for production."""
    ...
    def __init__(self):
        ...
        if tuned_path and tuned_path.exists():
            self.info_label.setText(
                f"✓ Tuned thresholds ({self.current_model})")
        elif self.good_stats:
            self.info_label.setText(
                f"✓ σ-based thresholds ({self.current_model}, σ={self.sigma})")
        else:
            self.info_label.setText("⚠ No threshold data — using defaults")
    ...
    def _init_ui(self):
        ...
        # ── LEFT: source image + defect overlay ───────────────────────────
        left = QWidget()
        ...
        self.img_label.setMinimumSize(700, 400)
        self.img_label.setStyleSheet(
            "background:#2b2b2b; color:#aaa; font-size:16px; border-radius:6px;")
        left_lay.addWidget(self.img_label, stretch=1)

        # Defect model prediction overlay (shown for PASS images)
        self.pred_overlay_label = QLabel("Defect model prediction (PASS images)")
        self.pred_overlay_label.setAlignment(Qt.AlignCenter)
        self.pred_overlay_label.setMinimumHeight(250)
        self.pred_overlay_label.setStyleSheet(
            "background:#1a1a2e; color:#666; font-size:12px; border-radius:6px;")
        left_lay.addWidget(self.pred_overlay_label, stretch=1)
        ...
        # --- O-Ring Model buttons (replaces dropdown) ---------------------
        model_group = QGroupBox("O-Ring Model")
        model_lay = QHBoxLayout()
        self.model1_btn = QPushButton("Model 1")
        self.model2_btn = QPushButton("Model 2")
        self.model1_btn.setCheckable(True)
        self.model2_btn.setCheckable(True)
        self.model1_btn.setStyleSheet(
            "font-size:14px; padding:8px 16px; font-weight:bold;")
        self.model2_btn.setStyleSheet(
            "font-size:14px; padding:8px 16px; font-weight:bold;")
        # Set initial state
        if self.current_model == "Model 1":
            self.model1_btn.setChecked(True)
        else:
            self.model2_btn.setChecked(True)
        self._update_model_btn_styles()
        self.model1_btn.clicked.connect(lambda: self._on_model_btn("Model 1"))
        self.model2_btn.clicked.connect(lambda: self._on_model_btn("Model 2"))
        model_lay.addWidget(self.model1_btn)
        model_lay.addWidget(self.model2_btn)
        model_group.setLayout(model_lay)
        right_lay.addWidget(model_group)

        # --- Detection controls -------------------------------------------
        sg = QGroupBox("Detection")
        sl = QVBoxLayout()
        ...
        # --- Metrics table (5 columns — no checkboxes, no category) ------
    ...
    def _populate_table(self):
        """Fill table rows with metric names and threshold spinboxes."""
        for row, (key, name, unit, ttype, dec, step, s_lo, s_hi, category) in \
                enumerate(METRIC_DEFS):
            label = f"{name}" + (f"  ({unit})" if unit else "")
            cat_tag = "R" if category == "rework" else "X"

            # Col 0 – metric name with category tag
            item0 = QTableWidgetItem(f"[{cat_tag}] {label}")
    ...
    def _update_model_btn_styles(self):
        """Visual toggle effect on the Model 1 / Model 2 buttons."""
        active = (
            "font-size:14px; padding:8px 16px; font-weight:bold; "
            "background:#1976D2; color:white; border:2px solid #1565C0;")
        inactive = (
            "font-size:14px; padding:8px 16px; font-weight:bold; "
            "background:#3a3a3a; color:#aaa; border:1px solid #555;")
        self.model1_btn.setStyleSheet(
            active if self.model1_btn.isChecked() else inactive)
        self.model2_btn.setStyleSheet(
            active if self.model2_btn.isChecked() else inactive)

    def _on_model_btn(self, model_name: str):
        """Handle Model 1 / Model 2 button click."""
        self.model1_btn.setChecked(model_name == "Model 1")
        self.model2_btn.setChecked(model_name == "Model 2")
        self._update_model_btn_styles()
    ...
    def _sync_thresholds_to_table(self):
        """Push self.thresholds → spinboxes."""
    ...
    def _ensure_yolo_detector(self) -> bool:
        """Lazy-load the YOLO model. Returns True if ready."""
    ...
    def _run_yolo_on_pass(self):
        """Run YOLO on a geometrically-passing image. Returns True if defects found."""
        if self.image is None:
            return False
        if not self._ensure_yolo_detector():
            self.pred_overlay_label.setText("⚠ YOLO model not available")
            return False
        ...
        try:
            ...
            # Build overlay with predictions
            ...
            self.pred_overlay_label.setText(f"⚠ YOLO error: {e}")
            return False
        finally:
    ...
    def _bin_crop_720(self, image: np.ndarray) -> np.ndarray:
        """2×2 bin + BG crop + resize/pad to 720×720."""
    ...
    def _run_maskrcnn_on_pass(self):
        """Run Mask R-CNN on a geometrically-passing image. Returns True if defects found."""
        if self.image is None:
            return False
        ...
        try:
            img_720 = self._bin_crop_720(self.image)
            pred = self._detector.predict(img_720)
    ...
    def _auto_bg(self):
        ...
        self.info_label.setText(f"Auto BG: {val}")
    ...
    def _update_nav_buttons(self):
        ...
        self.nav_label.setText(f"{idx + 1} / {n}" if n > 0 else "")
    ...
    def analyze(self):
        ...
        self.result_normed = normalize_measurements(
            self.result, self._resolution_scale)

        # Display overlay on the source image panel
        ...
        self.info_label.setText(
            f"R_out={r['outer_radius']:.1f}  R_in={r['inner_radius']:.1f}  "
            f"Cdist={r['center_dist']:.1f}  Ecc={r['eccentricity_pct']:.2f}%")
    ...
    def _evaluate(self):
        ...
        self.pred_overlay_label.setText(
            f"{self._defect_model} prediction (PASS images)")
        ...
        if rework_fails:
            ...
            detail = f"Shape ({len(rework_fails)}): " + ", ".join(rework_fails)
            if reject_fails:
                detail += f" | Also failing: " + ", ".join(reject_fails)
            ...
            # Geometric PASS → run defect model for confirmation
            ...
            if has_defect:
                ...
                self.verdict_detail.setText(
                    f"All metrics OK • {model_name}: no defects")
    ...
    def _load_best_thresholds(self) -> Dict[str, Dict]:
        """Load tuned JSON if available, else σ-based. Widen REJECT by 10%."""
        tuned_path = TUNED_JSON.get(self.current_model)
        if tuned_path:
    ...
    def _on_model_changed(self, model_name: str):
        ...
        if tuned_path and tuned_path.exists():
            self.info_label.setText(f"✓ {model_name} — tuned thresholds")
        elif self.good_stats:
            self.info_label.setText(f"✓ {model_name} — σ={self.sigma}")
        else:
            self.info_label.setText(f"⚠ {model_name} — using defaults")
    ...
    def _init_camera(self) -> bool:
        if not HIKROBOT_AVAILABLE:
            QMessageBox.warning(
                self, "Camera Error", "Hikrobot SDK not available.")
            return False
        ...
        try:
            ...
            if ret != 0 or deviceList.nDeviceNum == 0:
                QMessageBox.warning(
                    self, "Camera Error", "No cameras found!")
                return False
    ...
    def _start_stream(self):
        ...
        self.info_label.setText("Camera streaming — press Capture")
    ...
    def _grab_frame(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if px == 0x01080001:          # Mono8
                ...
                frame = cv2.cvtColor(raw.reshape(h, w), cv2.COLOR_BayerGB2BGR)
            elif px == 0x0108000B:        # BayerGR8
                frame = cv2.cvtColor(raw.reshape(h, w), cv2.COLOR_BayerGR2BGR)
            elif px == 0x01080008:        # BayerBG8
                frame = cv2.cvtColor(raw.reshape(h, w), cv2.COLOR_BayerBG2BGR)
    ...
    def _capture_frame(self):
        ...
        if frame is None:
            QMessageBox.warning(
                self, "Capture Error", "No frame available.")
            return
